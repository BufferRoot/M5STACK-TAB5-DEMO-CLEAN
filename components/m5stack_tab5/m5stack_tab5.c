...
esp_err_t bsp_sdcard_init(char* mount_point, size_t max_files)
{
    ...
    if (pwr_ctrl_handle == NULL) {
        // Suppress benign 0mV warning from LDO acquire (ESP-IDF initializes adjustable LDO with 0mV)
        esp_log_level_set("ldo", ESP_LOG_ERROR);
        ret_val = sd_pwr_ctrl_new_on_chip_ldo(&ldo_config, &pwr_ctrl_handle);
        esp_log_level_set("ldo", ESP_LOG_WARN);
        if (ret_val != ESP_OK) {
...
static esp_err_t bsp_codec_es8388_set(uint32_t rate, uint32_t bps, i2s_slot_mode_t ch)
{
    ...
    static uint32_t cur_bps  = 0;
    static i2s_slot_mode_t cur_ch = (i2s_slot_mode_t)0;

    // Skip reconfiguration if settings haven't changed
    if (cur_rate == rate && cur_bps == bps && cur_ch == ch) {
        return ESP_OK;
    }

    esp_err_t ret = ESP_OK;

    // Close record device first (shares same I2S port, must match sample rate)
    ...
    // Reopen record device at matching sample rate
    if (record_dev_handle) {
        ...
        esp_codec_dev_open(record_dev_handle, &rec_fs);
    }
